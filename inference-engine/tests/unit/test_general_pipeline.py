...
_FIXED_DATE = datetime(2026, 1, 15, tzinfo=timezone.utc)
...
def _fake_clip(clip_id, score_aircraft=0, score_vehicle=0,
    ...
    clip.published_at = _FIXED_DATE  # real datetime — prevents MagicMock dir names
    return clip
...
def _run(clips, file_exists=True, validate_result=(True, 0.5), det_counts=None):
    ...
    try:
        with patch("tasks.annotate_clips._latest_weights", return_value=Path("/w/best.pt")), \
             ...
             patch("tasks.annotate_clips.finalize_clip", return_value="/out/abc_annotated.mp4"):
            return _run_general()
    finally:
