"""
training-engine/tasks/train_baseline.py
...
def _make_epoch_callback(run_id: int, total_epochs: int):
    """Singular alias used by unit tests — returns only the end-of-epoch callback."""
    _, on_fit_epoch_end = _make_epoch_callbacks(run_id, total_epochs)
...
def train_baseline(self, training_run_id: int) -> dict:
    ...
    if not yaml_path.exists():
        raise FileNotFoundError(
            ...
            f"Run first:  cd training-engine && python scripts/build_specialist_datasets.py --models {model_type.value}"
        )
