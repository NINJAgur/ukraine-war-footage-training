...
Run after all specialist pipelines. Picks up remaining DOWNLOADED clips that
weren't consumed by AIRCRAFT / VEHICLE / PERSONNEL (ties go to first specialist,
UAS-only clips, or low-score mixed-content clips).

Uses the GENERAL model (trained on all 3 classes) for both validation and
annotation. Sets det_class='GENERAL' on annotated clips.

Usage:
    cd training-engine && python scripts/general_pipeline.py [limit]
    from scripts.general_pipeline import run; run(limit=10)
"""
import sys
import logging
...
log = logging.getLogger("general_pipeline")
...
def run(limit: int = 10) -> dict:
    ...
    log.info("=" * 60)
    log.info("GENERAL PIPELINE START")
    log.info("=" * 60)

    weights_path = _latest_weights("GENERAL")
    ...
    with get_session() as session:
        ...
        for clip in candidates:
            ...
            log.info(
                ...
                f"personnel={clip.score_personnel} uas={clip.score_uas} pov={clip.is_pov}\n"
            ...
            _, det_counts = infer_video_multi_model(
                [(model, "GENERAL", COLOR)], str(raw_path), save_path=str(temp_out),
