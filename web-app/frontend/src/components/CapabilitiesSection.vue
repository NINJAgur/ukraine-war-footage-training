      ...
      <div class="model-dl-grid">
        <div v-for="m in bestModels" :key="m.model" class="model-dl-card">
          <div class="model-dl-top">
            ...
            <span class="model-dl-map mono" v-if="m.map50 != null">{{ m.map50.toFixed(3) }}<span class="map-label"> mAP50</span></span>
          </div>
          <div class="model-dl-desc mono">{{ m.classes?.covers }}</div>
          <a v-if="m.download_url" :href="m.download_url" class="model-dl-btn" download>best.pt &nbsp;~49MB</a>
